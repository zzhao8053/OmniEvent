...
func NewLocalFileSystemObjectStorage(pathPrefix string) (*LocalFileSystemObjectStorage, error) {
	storage := &LocalFileSystemObjectStorage{
		rootPath: filepath.Join(GetStorageConfig().LocalFileSystemPath, pathPrefix),
	}
...
func (s *LocalFileSystemObjectStorage) Exists(ctx *context.WebContext, path string) (bool, error) {
	return utils.IsExists(s.getFinalPath(path))
}

// Read returns the object instance according to specified the file path
func (s *LocalFileSystemObjectStorage) Read(ctx *context.WebContext, path string) (ObjectInStorage, error) {
	return os.Open(s.getFinalPath(path))
}

// Save returns whether save the object instance successfully
func (s *LocalFileSystemObjectStorage) Save(ctx *context.WebContext, path string, object ObjectInStorage) error {
...
func (s *LocalFileSystemObjectStorage) Delete(ctx *context.WebContext, path string) error {
