...
func (w *RotateFileWriter) rotateFile() error {
	...
	if err != nil {
		return fmt.Errorf("cannot close log file \"%s\", because %w", w.file.Name(), err)
	}
	...
	if err != nil {
		return fmt.Errorf("cannot rename log file \"%s\" to \"%s\", because %w", currentFileName, archiveFileName, err)
	}
...
func (w *RotateFileWriter) openFile() error {
	...
	if err != nil {
		return fmt.Errorf("cannot open log file \"%s\", because %w", w.filePath, err)
	}
