    ...
    <div class="layout-wrapper">
        <router-link to="/">
            <div class="auth-logo d-flex align-start gap-x-3">
                <img alt="logo" class="login-page-logo" src="/img/logo.svg" />
            ...
            <v-col cols="12" md="4" class="auth-card d-flex flex-column">
                <div class="d-flex align-center justify-center h-100">
                    <v-card variant="flat" class="w-100 mt-0 px-4 pt-12" max-width="500">
                        <v-card-text>
                            <h4 class="text-h4 mb-2">验证邮箱</h4>
                            <p class="mb-0" v-if="token && loading">验证中...</p>
                            <p class="mb-0" v-if="token && verified">邮箱已验证</p>
                            <p class="mb-0" v-if="token && !verified && errorMessage">{{ errorMessage }}</p>
                            <p class="mb-0" v-if="!token && !email">参数无效</p>
                            <p class="mb-0" v-if="!token && email">已向 {{ email }} 发送验证邮件，请查收</p>
                        </v-card-text>

                        <v-card-text class="pb-0 mb-6">
                            <v-form>
                                <v-row>
                                    <v-col cols="12" v-if="!loading && !token && email">
                                        <v-text-field
                                    ...
                                    <v-col cols="12">
                                        <router-link class="d-flex align-center justify-center" :to="verified ? '/' : '/login'"
                                            ...
                                            <span v-if="!verified">返回登录</span>
                                            <span v-else-if="verified">返回首页</span>
                                        </router-link>
                                    </v-col>
                                </v-row>
                            </v-form>
...
function init(): void {
    ...
    // TODO: implement verifyEmail API call
    ...
    loading.value = false;
    verified.value = false;
    errorMessage.value = '邮箱验证功能待实现';
}

function resendEmail(): void {
    resending.value = true;

    // TODO: implement resendVerifyEmailByUnloginUser API call
